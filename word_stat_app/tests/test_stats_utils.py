...
class TestsStatsUtils(unittest.TestCase):
    ...
    def test_get_html_check_connection(self):
        self.assertIsNotNone(stats_utils.get_html('http://www.example.org'))

    def test_get_html_(self):
        with patch('stats_utils.requests.get') as mock_get:
            mock_get.return_value.ok = True
            mock_get.return_value.text = "Succes"
            response = stats_utils.get_html('justsomeadress')
            mock_get.assert_called_with('justsomeadress')
            self.assertEqual(response, "Succes")

            mock_get.return_value.ok = False
            response = stats_utils.get_html('anothereadress')
            mock_get.assert_called_with('anothereadress')
            self.assertEqual(response, None)


    def test_parse_html(self):
        self.assertIsNotNone(stats_utils.parse_html(self.html_kw))
        self.assertIsNotNone(stats_utils.parse_html(self.html_no_kw))

    def test_find_keywords(self):
        parsed_html_kw = BeautifulSoup(self.html_kw, 'lxml')
        parsed_html_no_kw = BeautifulSoup(self.html_no_kw, 'lxml')

        self.assertListEqual(stats_utils.find_keywords(parsed_html_kw), self.words)
        self.assertListEqual(stats_utils.find_keywords(parsed_html_no_kw),[])

    def test_create_dict(self):
        self.assertDictEqual(stats_utils.create_dict(self.words), self.result_dict)


    def test_count_words(self):
        html_body = """There are some dummy keywords Just for testing Some, some, for testing keywords"""
        with patch ('stats_utils.create_dict') as mock_dict,\
            patch ('stats_utils.get_body_text') as mock_text:
            mock_dict.return_value = self.result_dict
            mock_text.return_value = html_body

            self.assertDictEqual(stats_utils.count_words('whatever',self.words),
    ...
    def test_get_body_text(self):
        ...
        self.assertEqual(stats_utils.get_body_text(parsed_html_kw), parsed_html_no_kw.body.get_text())
        self.assertEqual(stats_utils.get_body_text(parsed_html_no_kw), parsed_html_no_kw.body.get_text())
